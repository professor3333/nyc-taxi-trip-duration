...
- Proof: `uv run pytest tests/test_registry.py`, 40 tests. Failure injection at 3 prepare points and 7 committed transitions, for promote and rollback; recover and abort after each; recover interrupted itself; md5 mismatch refused. Also exercised against the real Compose registry with `promote.py --refresh` (S3 download, md5 check, staged install). Its output was not committed: it would redeploy. The committed fixture is the pre-#53 6-dp rounding of the same values (max diff 0 at 6 dp).
