...
## Pipeline inputs audited — 2026-09-23

- `dvc.yaml` now declares the environment (`uv.lock`, `.python-version`) on every stage, every transitively imported module (prepare gains `validate.py` and `ingest.py`; evaluate gains `train.py` and `config.py`) and exactly the params each stage reads (`data.*` keys, including `data.reference_dir` for prepare, train and evaluate). Paths are templated from params. Stages run with `uv run --locked`, so a `pyproject.toml` dependency edit cannot bypass the `uv.lock` dep (it exits 2). `pyproject.toml` itself is not a dep, so lint config edits don't retrain.
- `params_hash` covers only train's params (it hashed all of params.yaml, an undeclared input). `n_threads` now overrides an inherited `OMP_NUM_THREADS` (it was `setdefault`).
- Proof: `tests/test_dvc_deps.py` has 29 tests, static coverage plus `dvc status` in an isolated repo for 13 kinds of change (uv.lock → all 5 stages; `api.*` → none). 20 of the 29 fail against the old dvc.yaml. Audit table: `docs/pipeline_inputs.md`.
- Lock regenerated by a full `dvc repro` (macOS arm64). Metrics are identical to the CI-produced lock; processed features differ by ≤ 1 ulp (`np.hypot`, libm vs glibc). The run's MLflow id points at a throwaway SQLite store: the model is unchanged, so no registry run was made.

## Reproducibility checks made honest — 2026-09-23 (PR A: code; outputs regenerate next)

- **Comparator** (`scripts/compare_run.py`): NaN/inf/unparsable values fail on either side at any tolerance (the old one returned "no differences" for a NaN prediction, confirmed). Metric keys missing on either side fail. Expected results come from `git show <sha>:<path>`, not a working tree. Default tolerance 0: bit-identical.
- **Precision:** fixture predictions are stored unrounded (they were rounded to 6 dp, so the 1e-9 comparison meant nothing below 5e-7).
- **Provenance:** train no longer reads the half-updated `dvc.lock`. It records `inputs_md5` of the files it opened; the completed-run record is `dvc.lock` at the commit (register.py → `dvc_lock_md5`).
...
## Promotion is a transaction; the fallback version covers its bucket edges — 2026-09-23

- **Reproduced first:** a fixture-generation failure during `promote` left alias `champion` = v2 while `champion.json` said v1 (the alias was set before anything else could fail). Rollback had the same ordering.
- **Now (ADR-0007 amendment):** prepare (md5-check the downloaded artefacts against the registered tags, stage all four release files, copy the current ones aside), then journal (the commit point), then aliases, then atomic per-file installs. Before the journal a failure changes nothing; after it every command refuses until `make promote-recover` (idempotent) or `make promote-abort` (exact prior state). The consistency check also requires the fixture to match `fixture_sha256`.
- Proof: `uv run pytest tests/test_registry.py`, 40 tests. Failure injection at 3 prepare points and 7 committed transitions, for promote and rollback; recover and abort after each; recover interrupted itself; md5 mismatch refused. Also exercised against the real Compose registry with `promote.py --refresh` (artefacts downloaded through the MLflow server from its local volume, md5 check, staged install). Its output was not committed: it would redeploy. The committed fixture is the pre-#53 6-dp rounding of the same values (max diff 0 at 6 dp).
- **Fallback identity** now hashes the medians plus the hour-bucket edges, `min_count` and the levels. The same medians under other edges predicted 10 vs 30 min under one version. It also no longer changed across save→load (`n` came back as float). The served `fallback_version` string changes once.
- **Not done here:** registry backups with a demonstrated restore and preserving CI tracking records (next PR). The selected-vs-deployed distinction is draft PR #49's release ledger (blocked on the one-time AWS migration).

## Registry state is recoverable; CI tracking runs are kept — 2026-09-23
