...
| rollback target missing (image expired, or no verified release of that model) | deploy refuses before touching traffic: `the live release's image … is gone from ECR`, or `no verified release of vN is recorded. Refusing to rebuild it silently` | `deploy.yml` error annotation | `tests/test_releases.py`; keep-* ECR rule | Rollback: restore by `release_id`, or promote deliberately (a new release) |
